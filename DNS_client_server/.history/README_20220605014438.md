...
Our [DNSServer.py](https://github.com/claudiatang/network_programming_python/blob/main/DNS_client_server/DNSServer.py) needs a non-blocking UDP socket, as we do not want the listening function ```socket.recvfrom()``` block the "Ctrl + C" keyboard interruption function 