...
Our [DNSServer.py](https://github.com/claudiatang/network_programming_python/blob/main/DNS_client_server/DNSServer.py) needs a non-blocking UDP socket. Otherwise the listening function ```socket.recvfrom()``` blocks the execution of other part of the code when there is no data received from any client, which would disable the "Ctrl + C" keyboard interruption function and make the it impossible to close the running server   