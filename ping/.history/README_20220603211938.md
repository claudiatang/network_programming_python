...
- #### Receive echo reply
  ...
  -  The received ICMP message is encapsulated in an IP datagram where the 20-27th bytes are the ICMP header. Once extracted, the ICMP header bytes object is unpacked using ```struct.unpack()``` into fields using the same format as packing an ICMP message.
